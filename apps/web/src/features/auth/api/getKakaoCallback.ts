import { api, ENDPOINT, type ApiResponse } from '@/shared/api';
 ...
 *
 */
export const getKakaoCallback = async (code: string): Promise<ApiResponse<AuthResponse>> => {
  const response = await api.get<ApiResponse<AuthResponse>>(
    `${ENDPOINT.AUTH.KAKAO_CALLBACK}?code=${code}`,
    {
      credentials: 'include',
    }
  );
